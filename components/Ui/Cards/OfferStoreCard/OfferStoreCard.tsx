...
const OfferStoreCard = ({
  ...
  price,
  time,
  url,mainList,woocommerceList
}: OfferStoreCardPropsType) => {

  const mainListMap = <div className={styles.listBox}>
  ...
  const wordpressListMap = woocommerceList.length>0?<div className={styles.listBox}>
    <div className={styles.listHeader}>
      <img src={WoocommerceIcon.src} alt='woocommerce' width={30} height={30}/>
          <p>Wykonanie w WooCommerce:</p>
    </div>
    {woocommerceList.map((item,index)=><div key={index} className={styles.listItem}>
