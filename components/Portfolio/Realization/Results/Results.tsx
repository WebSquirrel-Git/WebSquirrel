...
export const Results = ({results}:ResultsProps) =>{
    ...
    <h2>Efekty</h2>
    <>
    {results.map((result,i)=><div key={i} className={styles.targetBox}>
        <Image  alt='checkmark' src={ResultGreen}/>
        <p>{result}</p>
    </div>)}
    </>
    
    </div>
    <Image className={styles.image} alt='checkmark' src={ResultOrange}/>
    </div>
}